...
#[test]
fn changing_slash_rate_after_dispute_opens_does_not_affect_it() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let client = EscrowContractClient::new(&env, &contract_id);
    client.initialize(&admin);
    client.set_juror_params(&admin, &DEFAULT_MIN_JUROR_STAKE, &3, &1000, &i32::MIN, &0); // 10% slash
    ...
    // admin cranks the slash rate up to 50% *after* the dispute is already
    // open - jurors voted (or are about to) under the 10% they saw
    client.set_juror_params(&admin, &DEFAULT_MIN_JUROR_STAKE, &3, &5000, &i32::MIN, &0);
    ...
    // still 10% (the rate at raise_dispute time), not the 50% set later
    assert_eq!(client.get_juror_stake(&minority).unwrap().amount, 90_0000000i128);
}

#[test]
fn toggling_stake_weighted_voting_after_dispute_opens_cannot_flip_the_outcome() {
    let env = Env::default();
    env.mock_all_auths();
    ...
    let client = EscrowContractClient::new(&env, &contract_id);
    client.initialize(&admin);
    // flat voting when the dispute opens
    assert!(!client.get_stake_weighted_voting());
    ...
    // admin flips on stake-weighted voting *after* the dispute already
    // opened under flat voting
    client.set_stake_weighted_voting(&admin, &true);
    ...
    // still decided by headcount (2-1 renter, the mode this dispute
    // actually opened under) - the later toggle to stake-weighted must
    // not retroactively hand it to the whale-backed host side
    let dispute = client.get_dispute(&escrow_id, &0).unwrap();
    assert_eq!(dispute.outcome, DisputeOutcome::RenterWins);
}
